...
export const EnhancedCryptoWalletModal: React.FC<CryptoWalletModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleWalletSelect = async (walletOption: WalletOption) => {
    ...
    try {
      if (walletOption.connectViaPrivy) {
        console.log('🚀 Attempting to connect Phantom via Privy...');
        setConnectionStatus(`Connecting ${walletOption.name} via Privy...`);
        ...
        if (!walletOption.isAvailable) {
          throw new Error('Phantom wallet is not installed. Please install Phantom wallet extension first.');
        }

        // Try to connect via Privy
        const result = await connectWallet();
        console.log('✅ Privy connection result:', result);
        
        setConnectionStatus('Connected successfully!');
        setTimeout(() => onClose(), 1000);
        
      } else {
        console.log(`🚀 Attempting to connect ${walletOption.name} via Solana adapter...`);
        ...
        // Find wallet in available adapters
        ...
        if (wallet) {
          ...
          console.log(`✅ Connected ${walletOption.name} via Solana adapter`);
          setConnectionStatus('Connected successfully!');
          setTimeout(() => onClose(), 1000);
        } else {
          console.warn(`⚠️ ${walletOption.name} adapter not found, trying Privy fallback...`);
          // Fallback: try to connect via Privy if adapter not found
          await connectWallet();
          setConnectionStatus('Connected via Privy fallback!');
          setTimeout(() => onClose(), 1000);
        }
      }
    } catch (error: any) {
  ...
  const handleInstallWallet = (walletName: string) => {
    ...
    const url = urls[walletName as keyof typeof urls];
    if (url) {
        ...
        {connectionStatus && (
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium text-blue-800">{connectionStatus}</span>
            </div>
          </div>
        )}
          ...
          {walletOptions.map((wallet) => (
            <div key={wallet.name} className="relative">
              <button
                ...
                disabled={isConnecting}
                className={`w-full flex items-center px-4 py-4 border rounded-lg transition-all duration-200 group ${
                  wallet.isAvailable 
                    ? 'border-gray-300 hover:bg-gray-50 hover:border-purple-300' 
                    : 'border-gray-200 bg-gray-50'
                } ${isConnecting ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                <div className={`flex items-center justify-center w-12 h-12 mr-4 rounded-lg transition-colors ${
                  wallet.isAvailable 
                ...
                <div className="flex-1 text-left">
                  <div className="flex items-center">
                    <h3 className={`font-medium ${wallet.isAvailable ? 'text-gray-900' : 'text-gray-500'}`}>
                      {wallet.name}
                    </h3>
                    {wallet.connectViaPrivy && wallet.isAvailable && (
